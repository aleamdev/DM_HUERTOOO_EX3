...
import com.dewis.dm_huertohogar_ex3.data.remote.CarritoItemRequestDto
import com.dewis.dm_huertohogar_ex3.data.remote.CarritoResponseDto
...
class CartViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun removeItem(token: String?, productoId: Long) {
        if (token.isNullOrBlank()) {
            _ui.value = _ui.value.copy(error = "Debes iniciar sesión para modificar el carrito.")
            return
        }
        ...
        viewModelScope.launch {
            try {
                _ui.value = _ui.value.copy(loading = true, error = null, success = null)
                val cart = ApiClient.cartApi.eliminarItem(auth(token), productoId)
                _ui.value = _ui.value.copy(
                    loading = false,
                    cart = cart,
